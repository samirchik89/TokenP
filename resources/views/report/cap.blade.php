    ...
    <div class="content-area py-1">
        ...
        <div class="container-fluid">
            ...
            <table class="table table-striped table-bordered dataTable user-list-table" id="table-2" style="width: 100% !important">
                ...
                <tbody id="table-body">
                    @foreach ($customersWithShares as $index => $customer)
                        <tr>
                            <td>{{ $customer->name }}</td>
                            <td>{{ $customer->country->countryname }}</td>
                            <td>{{ $customer->investorShares->internal_wallet }}</td>
                            <td>{{ $customer->investorShares->external_wallet }}</td>
                            <td>{{ $customer->investorShares->internal_wallet + $customer->investorShares->external_wallet }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
