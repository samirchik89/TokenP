...
<div class="content-area py-1">
    <div class="container-fluid">
    	<div class="box box-block bg-white">
    	    <a href="{{ route('admin.tokendocument.index') }}" class="btn btn-default pull-right"><i class="fa fa-angle-left"></i> @lang('admin.back')</a>

			<h5 style="margin-bottom: 2em;">@lang('admin.update_doc')</h5>

            <form class="form-horizontal" action="{{route('admin.tokendocument.update', $document->id )}}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}
            	<input type="hidden" name="_method" value="PATCH">
				<div class="form-group row">
					<label for="name" class="col-xs-2 col-form-label">@lang('admin.doc_name')</label>
					<div class="col-xs-10">
						<input class="form-control" type="text" value="{{ $document->document_name }}" name="document_name" required id="title" placeholder="Document Name">
					</div>
				</div>
                ...
                <div class="form-group row">
                    <label for="order" class="col-xs-2 col-form-label">@lang('admin.status')</label>
                    <div class="col-xs-10">
                        <select class="form-control" id="status" name="status" required>
                        	<option value="0" @if($document->status==0) selected @endif >Inactive</option>
                        	<option value="1" @if($document->status==1) selected @endif >Active</option>
                        </select>
                    </div>
                </div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						...
						<a href="{{route('admin.tokendocument.index')}}" class="btn btn-default">@lang('admin.cancel')</a>
					</div>
				</div>
			</form>
		</div>
